...
several microbenchmarks were devised:
...
\item
  ...
  of primitive types.
\item
  \emph{getCurrentTime} benchmarks the implementation of
...
The resulting programs, compiled with the Haste compiler, were then repeatedly
executed using the Node.js JavaScript interpreter, and the average run times
of the programs using our interface compared against the average run times of
their FFI counterparts.
...
although much less noticable, a ``mere'' 300\% speedup. Inspecting the
generated code, we see that this is caused by the loop in question having a
...
reduce the relatively performance penalty - however slight - incurred by our
