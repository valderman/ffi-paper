...
old one. We decompose interactions with the host environment into its
constituent parts: marshalling arguments into the target language,
performing the actual foreign call,
and finally marshalling the results back into Haskell.
...
distributed in a high level browser context and traditional,
down-to-the-metal one.
...
The idea of a higher level foreign function interface is by no means novel in
itself; there already exists a large body of work in this problem domain,
as we discuss in sectoin \ref{sec:related}. However, to our knowledge, our
solution is the first to address all of the aforementioned criteria.
...
abstract \lstinline!HostAny! data type, inhabited by web-native
...
Fortunately, our JavaScript host language, as well as virtually any other
...
of the vanilla FFI - easily accomplished by adding a newtype wrapper for
\lstinline!Double! with the appropriate \lstinline!FromAny! instance -
we see the same performance and generated code
as in the \emph{tight loop, no inbound data} case.

Thus, although we prefer to err on the side of safety for the default
marshalling of numbers, the choice on whether to use the slower, safer
marshalling or the faster, unsafe one is ultimately left to the user.
...
Judging by these numbers, disregarding the cost incurred by
the additional safety measures in the \emph{tight loop, with inbound data} case,
the expensive safety measures being completely optional and not a required
part or our framework, the performance of our library is quite acceptable,
