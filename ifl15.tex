...
there already exists a large body of work in this problem domain, solving most
of the problems of \ref{lst:bad-ffi}, which is used here as an example mainly to
...
\item
  ...
  of primitive types.
\item
  \emph{Product types}, which benchmarks the implementation of
...
Unfortunately, this usefulness comes at the price of a more complex
implementation. Not only does quasiquoting require explicit compiler support,
but FFI implementors need to supply a parser for the quoted language as
well. A price in implementation effort that may not always be worth the extra
expressive power it buys. While the static, compile time nature of quasiquoting
gives the opportunity to provide type safety for quoted languages, this
advantage is rather wasted on JavaScript and other dynamically typed languages.
