...
class Import f
instance (Marshal a, Import b) => Import (a -> b)
instance Marshal a             => Import (IO a)

-- Instances for functions and basic types
instance Marshal Int
...
instance Import f => FromAny f
instance (Marshal a, Exportable b) => Marshal (a -> b)
instance Marshal a => Marshal (IO a)
...
\begin{itemize}
\item
  define the appropriate \lstinline!Marshal! instances
...
\item
  ...
  any set of types instantiating \lstinline!Marshal!,
...
\begin{code}
instance Marshal HostAny where
  toAny = id
  fromAny = return
\end{code}
...
\lstinline!arrToList :: Marshal a => HostAny -> IO [a]!
and \lstinline!listToArr :: Marshal a => [a] -> HostAny!
which are used to implement the \lstinline!Marshal! instance for lists;
...
getMember :: Marshal a => HostAny -> String -> IO a
getMember dict key =
...
\begin{listingfloat}
\begin{code}
instance (Marshal a, Marshal b) =>
         Marshal (Either a b) where
...
instance Marshal a => Import (IO a) where
  import_ f args =
