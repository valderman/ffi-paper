...
standard libraries of GHC; a ``simple'' piece of code to retrieve the current
time\ \cite{time}.
...
This code snippet is more akin to thinly veiled C code than proper, readable
...
shackles of the low level Foreign Function Interface used to interface
Haskell with C since time immemorial.
...
\begin{code}
  ...
  getCurrentTime =
    host "() => {var now = window.performance.now();\\
                \\return {msecs: Math.trunc(now),\\
                \\        usecs: Math.trunc(now*1000)};}"
\end{code}
...
\begin{itemize}
\item
  The low level C types are gone, replaced by a nice, descriptive record type,
...
In section \ref{sec:haste-foreign}, we present a novel interface for a hosted
...
where the C calling conventions have reigned unchallenged since time
immemorial. The C language has no notion of higher level data structures or
...
\item
  ...
  save the module's consumer from having to deal with these details, as done
...
\section{A New Way}\label{sec:haste-foreign}
...
\subsection{Crossing the Rift}\label{sec:interface}
...
arguments. As shown in figure \ref{lst:specialization}, in addition to the
general \lstinline!apply! function we define a series of \lstinline!apply0!,
\lstinline!apply1!, etc. functions, one for each arity we want to optimize
function application for. The actual specialization is then a mere matter of
...
Creating a \lstinline!safe_host! function which can not be used to execute
user-provided code becomes quite easy using this extension, at the cost of
slightly more inconvenient import syntax:

\begin{code}
  safe_host :: Import f => StaticPtr HostString -> f
  safe_host = host . deRefStaticPtr

  safe_alert :: String -> IO ()
  safe_alert = safe_host $ static "alert"
\end{code}
...
Finally, by eliminating all occurrences of \lstinline!eval!, we may
extend the usefulness of our interface to platforms which do not support
evaluating code at runtime as well, most notably the JVM.
...
is applied to a statically known string, do not generate the function call,
but splice the contents of the string verbatim into the output code instead.

This solution has the advantage of eliminating the code analysis obstacle
provided by \lstinline!eval! for the case when our target code is statically
known (which, as we noted before, is a basic sanity property of foreign
imports), while preserving our interface's simplicity of implementation.

By combining this approach with the method for restricting our interface to
statically known strings described in section \ref{sec:safe_host},
we guarantee the complete absence of \lstinline!eval! from the generated host
code, removing the restriction that our host language supports dynamic code
evaluation at runtime, notably making our interface implementable on
recent versions of the Java Virtual Machine.

\section{Extending the interface}
...
\item
  ...
  its argument as a function to be converted into a host language function!
\end{itemize}
...
\lstinline!Exportable! for any \lstinline!ToAny!, and the
\lstinline!ToAny t => Exportable t! instance provides \lstinline!ToAny!,
creating a loop which creates instances for both type classes matching any
type.
...
Here we also see why the use of closed type families are necessary:
the open type families originally introduced by Chakravarty et al
...
\ref{sec:marshalling-functions}, implementing \lstinline!export! becomes
quite simple.
...
\begin{listingfloat}
  \begin{code}
    ...
    > printAny = ffi "x => {console.log(x.toString());}"
...
allow us to mix pure functions with ``probably kind of pure, maybe''
functions, and for good reason - it is not clear that we should lift this
...
which has been surprisingly rarely described outside a GHCJS context.
...
While the interface presented in this paper is implementable across a wide
range of host environments, particularly with the modification described in
section \ref{sec:inline-eval} to lift the restriction of the host language
supporting a dynamic evaluation construct, we have yet to implement this
interface for a non-JavaScript environment.
